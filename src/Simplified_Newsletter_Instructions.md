...
**Examples:**
- ✓ "Improved user feedback process" (instead of: "Completed 35 UAT tickets with 8 resolved")
- ✓ "Feature development on track" (instead of: "76 story points in sprint")
- ✓ "Enhanced system reliability" (instead of: "Fixed JSON parsing for API")
...
**Title format:** � <Month Year> Engineering Newsletter

Derive month and year from JSON "date", using the next month (e.g., "2025-09-01" — "October 2025").

**Icon:** 📢 (announcement megaphone) for contextual messaging and strategic direction. This may be swapped for a contextual icon based on newsletter content (e.g., 🎯 for strategic focus, 🚀 for launches, etc.).
...
- Example: 13 projects + 1 "Quick Wins" = 14 items — Left: 0-6 (7 items, including "Quick Wins"), Right: 7-13 (7 items)
...
- **Status-based coloring:**
  - If status === "Done", "Deployed", or "Live in Production": Bold, green #28a745, with '✓' prefix, and **link color must also be #28a745**
  - Otherwise: Regular text, color #333, no checkmark, link color #333

**Links:** Each item is an anchor link (href="#project-id") where project-id is the project title in lowercase, spaces replaced with hyphens, special characters removed (e.g., "Client Portal - Phase 1" — "client-portal-phase-1", "Bidding 2.2" — "bidding-2-2"). "Quick Wins" links to #quick-wins.
...
- Example: 14 cards — Left: 0-6 (7 cards), Right: 7-13 (7 cards)
...
1. **Project Title with Icon** (format: `<emoji> <Title> — <Status>`)
...
1. First, check project status:
  - If status is "Done", "Deployed", or "Live in Production" — use completion icons
  - If status is "UAT", "Testing", or "Final Refinements" — use testing icons
  - Otherwise — use in-progress icons

2. Then, match to project domain by searching for keywords in the title:
  - Finance/Payment keywords (discount, payment, billing, invoice, accrual, wallet, revenue): 💳
  - Communication/Call keywords (call, phone, voice, recording, consent, audio): 📞 or 🎧
  - Data/Analytics keywords (data, analytics, report, statistics, intel, feasibility): 📊
  - AI/Automation keywords (AI, automation, intelligent, smart, bot): 🤖
  - Web/Portal keywords (portal, website, web, client-facing): 🌐
  - System/Technical keywords (system, infrastructure, database, migration): ⚙️
  - Planning/Process keywords (planning, process, workflow, recruitment): 📋
  - Testing/Refinement keywords (testing, UAT, refinement): 🧪 or 🔧
  - Completion keywords (done, deployed, complete, live): 🎯
...
- If status is "Done", "Deployed", or "Live in Production" — • Key Achievements
- If status is "UAT", "Testing", or "Final Refinements" — 🧪 Testing Updates
- Otherwise — 🔄 Progress Updates

**Closing Statement Icons:**

Search the closing_statement text for keywords and return a suitable icon:
- Completion words (complete, done, finished, ready, delivered, deployed, live): 🏁 or •
- Launch words (launch, launching, release): 🚀
- Timing words (soon, scheduled, upcoming, next week, waiting): ⏳ or 📅
- Progress words (progress, advancing, developing, building): 🚧 or 📈
- Technical words (technical, development, engineering): 🔧 or ⚙️
- Goal words (target, goal, milestone, aim): 🎯
- Communication words (communication, feedback, updates): 🗣️ or 📞
...
1. **Centered Title** (format: `‼ Quick Wins — <Previous Month> Highlights`)
   - Style: `font-family:Arial,sans-serif;font-size:18px;color:#007bff;font-weight:bold;text-align:center;margin-bottom:15px`
   - Derive previous month from JSON "date" (e.g., "2025-09-01" — "September")
...
- Entry point: open the landing page at `/` and enter `from` and `to` dates (YYYY-MM-DD) — the form now submits to `/preview.html` which renders the final newsletter HTML for the requested range.
...
- Visual/TOC rules enforced:
  ...
  - Done-like statuses (`Done`, `Deployed`, `Live in Production`, `Released`) are flagged as `done` and rendered with a ✓ prefix and green link color (#28a745) in the TOC.
...
- Icons: icons are chosen dynamically by the mapper using this precedence: provided icon in JSON → status-based icon → label-based heuristics → title/keyword heuristics.
...
<table style="max-width:1200px;margin:0 auto;padding:20px;background:#f5f5f5;border-collapse:collapse;border-spacing:0;" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td style="background:#007bff;color:#fff;font-family:Arial,sans-serif;font-size:36px;font-weight:bold;padding:20px;border-radius:8px;text-align:center;" id="top">
      📰 October 2025 Engineering Newsletter
    </td>
  </tr>
      ...
      <table style="width:100%;border-collapse:collapse;border-spacing:0;" cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td width="50%" style="padding-right:20px;vertical-align:top;">
            <table style="width:100%;border-collapse:collapse;border-spacing:0;" cellpadding="0" cellspacing="0">
              <tr><td style="font-family:Arial,sans-serif;font-size:14px;color:#28a745;font-weight:bold;border-bottom:1px solid #f0f0f0;padding:4px 0;">✓ <a href="#project-id" style="color:#28a745;text-decoration:none;">Done Project Title</a></td></tr>
        ...
        <tr>
          ...
          <td width="50%" style="vertical-align:top;">
            ...
            <div style="background:#fff;padding:20px;border-radius:8px;border-left:5px solid #28a745;margin-bottom:20px;">
              <div style="font-family:Arial,sans-serif;font-size:18px;color:#007bff;font-weight:bold;margin-bottom:15px;">🎯 Project Title — Done</div>
              ...
              <div style="font-family:Arial,sans-serif;font-size:14px;color:#333;font-weight:bold;margin-top:10px;margin-bottom:10px;">• Key Achievements:</div>
              ...
              <div style="font-family:Arial,sans-serif;font-size:14px;color:#333;font-weight:bold;margin-top:10px;margin-bottom:15px;">🏁 Closing statement with impact and celebration!</div>
  ...
  <tr>
    <td>
      <div id="quick-wins" style="background:#e6f3fa;padding:30px;border-radius:8px;border:2px solid #007bff;">
        <div style="font-family:Arial,sans-serif;font-size:18px;color:#007bff;font-weight:bold;text-align:center;margin-bottom:15px;">‼ Quick Wins — September Highlights</div>
        ...
        <div style="font-family:Arial,sans-serif;font-size:14px;color:#333;font-weight:bold;margin-bottom:15px;">🔄 Recent Wins:</div>
        <ul style="margin:0 0 20px 20px;padding:0;font-family:Arial,sans-serif;font-size:14px;color:#333;line-height:1.6;">
          <li style="margin-bottom:8px;"><strong>Title</strong> — Description of business impact and value.</li>
        </ul>
        <div style="font-family:Arial,sans-serif;font-size:14px;color:#333;font-weight:bold;text-align:center;margin-bottom:15px;">‼ These quick improvements enhanced user experience and efficiency!</div>
